(function(zpr,$) {
	...
	function Service(hostUrl) {
		this.hostUrl = hostUrl;
	} 

	Service.prototype.getInitialStageData = function (stageId, callback) {
		$.ajax({
			method: "POST",
			url: "/game/map",
		...
		/*var text = '{ "name": "round1", "size": {"x":1000, "y":400}, "missle": { "start": {"x":50, "y":50} }, "objects":[] }';
		var data = JSON.parse(text);
		 callback(data/*{ 
			name: "round1", 
			size: {x:1000, y:400}, 
			missle: { start: {x:0, y:0}},
 			objects:[] 
 		}*//*);*/
	}

	Service.prototype.getActionData = function (input, callback) {
		$.ajax({
			method: "POST",
			url: "/game/simulate",
			data: 'shootX=' + input.x + '&shootY=' + input.y,
			success: function(data) {
				var data = JSON.parse(data);
				callback(data);
			}
	...
	Service.prototype.getObjectsData = function () {
		
	}

})(window.zpr ? window.zpr : window.zpr = {},jQuery)