(function () {
	angular
		...
		.constant('init-url', '/game/map')
		.constant('sim-url', '/game/simulate')
		.constant('auth-url', 'game/auth')
		.provider('CommServiceProvider', [
			CommServiceProvider
		]);

	var serverUrl = '';		

	function CommServiceProvider () {
	...
	function ServiceFactory($http, initUrl, simUrl, authUrl, httpResp) {
		var srv = {};

		srv.getInitialData = function(stageNumber, callback) {
			var data = {
				map: stageNumber
			}

			$http.post(serverUrl + initUrl, data).then(function (data) {
				callback(data);
			}, function (err) {
				callback(httpResp);
			});
		...
		srv.getSimilationData = function(missleData, callback) {
			var data = {
			    referencePoint: {
			    	x: missleData.x,
			    	y: missleData.y
		...
		srv.authenticate = function (user, pass, callback) {
			var data = {
				user: user,
				pass: pass
			}

			$http.post(serverUrl + authUrl, data).then(function (data) {
